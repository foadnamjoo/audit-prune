#!/usr/bin/env python3
"""CHPC job — re-encode the v1.1-edited cohort rows with BGE-Multi-Gemma2 and
...
Validation: re-encodes 3 UNEDITED rows per column and requires min cosine >= 0.9995 (relaxed from 0.9999: bf16 cross-GPU jitter ~1e-4 observed on grn084; recipe errors show as cos<0.99)
against the shipped v1.0 cache before splicing (same gate as the local families).

Run from the bundle root (see chpc_gemma2_job.md). Needs in the bundle:

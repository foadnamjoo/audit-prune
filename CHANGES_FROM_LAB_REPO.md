# Changes from the lab repository

Every file below is a verbatim copy of its lab-repo source except for the edits listed.
...
* `scripts/audit_halubench_contamination.py`: moved from rebuttal_2026/; repository root resolved relative to the file; output written to results/halubench_contamination_results.json
* `scripts/render_theta_sweep_figure.py`: removed a preview PNG written to the author's Desktop
...
* `scripts/run_aflite_baseline.py`: docstring run line uses `python` instead of a local interpreter path
* `scripts/run_random_subset_fidelity.py`: docstring run line uses `python` instead of a local interpreter path

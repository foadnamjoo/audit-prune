...
> Foad Namjoo, Remy Ogasawara, Amirali Abdullah, Cullen Anderson, Narmeen Fatimah Oozeer, Jeff M. Phillips. 2026.
...
data/predictions/                    per-item correctness of the 14-model open-weight panel (790 pairs each), the
                                     restyle/ check (8 open instruct models, 131 pairs x 2 renderings; ICLR-revision
                                     Appendix L, not in arXiv v1) and, under frontier/, the four closed models of Table 7
...
artifacts/embeddings/                cached frozen-encoder features (Hugging Face; not in git, 160 MB)
...
| Zero-shot restyling check (ICLR-revision Appendix L; not in arXiv v1) | `scripts/run_restyle_panel.py` (plain vs surface-inverted, shared A/B seed), `scripts/score_restyle_panel.py` (exact McNemar, Holm) | `data/predictions/restyle/*.csv` (per item, raw continuations), `data/predictions/restyle/summary.json` |
...
* Cached embeddings (`artifacts/embeddings/`, `results/v1_1_rescore/emb/`) are on the Hugging Face
  dataset, not in git; place them at those paths, or rebuild with the `build_*_embeddings.py` scripts.

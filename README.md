...
| Zero-shot restyling check (ICLR-revision Appendix L; not in arXiv v1) | `scripts/run_restyle_panel.py` (plain vs surface-inverted, shared A/B seed), `scripts/score_restyle_panel.py` (exact McNemar, Holm; `--drop-drift-flagged` gives the 91-pair restriction) | `data/predictions/restyle/*.csv` (per item, raw continuations), `data/predictions/restyle/summary.json` |
